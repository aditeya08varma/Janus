...
### Round two: it failed again, and the real signal turned out to be one I was throwing away

The very next live use of the deployed fix — the same exact query, same person, minutes after the fix had gone out — hit the recursion cap and failed again. Same phrasing as my local test runs almost verbatim, through round 4, then one extra round the local tests hadn't taken, tripping straight over `RECURSION_LIMIT=10`. LLM output isn't perfectly deterministic even at `temperature=0`; the same prompt can occasionally run one round longer, and a cap with zero margin turns "one round slower than usual" into a hard failure. Raised `RECURSION_LIMIT` from 10 to 16 (8 round-trips instead of 5) — cheap insurance that still catches a genuinely runaway loop, it just stops treating ordinary variance as one.

That bought margin, but not an explanation. Before tuning anything else, I added logging of the actual scores driving each retrieval — both Pinecone's bi-encoder similarity score (`retrieval.py`, `retrieve_with_cascade`) and the cross-encoder reranker's score (`rerank_docs`), which was being computed on every single call and then never used for anything except sorting. Re-ran the same query with logging on and finally had real numbers instead of a theory.

The reranker scores told a much sharper story than "results were weak." Two of the six searches in that turn came back with every one of their top-8 chunks scoring *negative* — the cross-encoder's own calibrated judgment that nothing in the batch was actually relevant to that specific query — and the code returned those eight irrelevant chunks anyway, formatted identically to a genuinely good match. But the first two searches, the broad ones ("DRS Drag Reduction System," "Active Aero X Mode Z Mode"), scored strongly positive and clearly succeeded. So why did the model keep going after that?

The query *text* across all six searches answered it: general DRS concept → general Active Aero concept → activation speed specifically → activation criteria specifically → activation zone specifically → the 1-second gap rule specifically. The model wasn't retrying a search that failed — it was decomposing "compare these two systems" into a checklist of individual facets and spending one search per facet, because a broad overview query doesn't surface every specific numeric threshold buried in a different part of the document. My "at most once per year" rule couldn't constrain this, because the axis it was reasoning along was never "year" — it was "which fact am I missing next."

Replaced the per-year framing with a flat, count-based one instead: *at most 4 `search_knowledge_base` calls total for the whole turn*, regardless of how many years or facets the question touches, then synthesize from whatever was retrieved. Re-verified with three more fresh runs against the real stack: 0 failures, though the model still doesn't obey the literal "4" — it used 6–8 calls across those runs. The reliability is coming from the combination of the nudge *and* the wider recursion-limit margin, not from either alone perfectly working as specified. Worth being honest about: this reduces the failure rate substantially, it doesn't make the model's tool-call planning perfectly disciplined.

### Why the prompt-based budget never really held, and the fix that actually does

A fair question raised after all this: are the "guardrails" just worded unclearly? No — the real issue is the mechanism, not the wording. A system prompt is a soft constraint: it biases what the model is likely to generate, but nothing checks compliance before an action happens. "At most 4 calls" sits in the same context window as everything else competing for influence, and it was losing to a much better-reinforced pull — the model's own drive to cite a specific, retrieved source for every specific claim (system prompt rule 4, and the project's whole "truth over creativity" ethos), which made "let me search once more for that exact figure" feel like the *correct* move each time, not a violation. On top of that, "4" requires the model to accurately tally its own past tool calls purely by re-reading a transcript — exactly the kind of implicit bookkeeping LLMs are unreliable at, especially with nothing in the conversation ever restating the running count.

`RECURSION_LIMIT` never had this problem, because it isn't a request — LangGraph forcibly ends the run at the step cap regardless of what the model wants. That's the actual fix: move enforcement from the prompt into the code, the same way. `graph.py` now tracks real `search_knowledge_base` calls since the most recent human message (`_kb_calls_this_turn`), and once the count reaches `MAX_KB_CALLS_PER_TURN` (4):

- `agent_node` stops offering the tool at all for that round — invokes a version of the LLM bound only to `search_web`, with a short system nudge ("you have used all available searches, answer now"), so continuing to search isn't a choice the model can make anymore, not a rule it's expected to remember.
- `tool_node` backstops the case where a single round bundles more calls than the remaining budget in one shot (the round-level check alone let this slip through — one run overshot to 6 real searches before this landed): it counts how many of *this round's* requested `search_knowledge_base` calls are still within budget and returns a "budget already used" message for the rest, without ever touching Pinecone for them.

Verified against the real stack: 3 fresh runs, 0 failures, and — checked directly against the retrieval-layer logs, not just the telemetry the frontend shows — **exactly 4 real searches executed per run, every time**. The earlier "4 total calls" prompt version used 6-8 despite being told 4; this version cannot exceed 4, because exceeding it is no longer physically possible, not because it's asking more firmly.

One more honesty fix that fell out of this: the frontend telemetry log was showing an "ANALYZING" line for every call the model *requested*, including ones `tool_node` was about to skip — so a user could see 6 "ANALYZING" lines when only 4 searches actually ran. `api.py`'s log-yielding now mirrors the same budget math and shows a `⚠️ SEARCH BUDGET REACHED, SKIPPING` line for anything past the cap instead of pretending it searched.
...
## Checking the actual quality, not just the bugs — running the eval harness for real

Everything above is about specific bugs found and fixed one at a time. After enough of those, the honest question is whether the *system* still works well overall, not just whether the last thing that broke got fixed. The repo already had two eval scripts (`run_evals.py`, an LLM-as-judge PASS/FAIL harness, and `run_ragas.py`, a Ragas faithfulness/relevancy/precision harness) and a 5-question golden set — but the last reports on disk were dated January 2026, from before any of this, and running them surfaced two real problems of their own.

First: neither script ever included `SYSTEM_PROMPT`. Both call `graph_builder.compile()` and hand it a bare `HumanMessage` — no year-defaulting rule, no citation format, no tool budget. The harness has never actually tested the agent configuration that's deployed; it's been testing a different, unconstrained agent this whole time. Fixed by extracting `SYSTEM_PROMPT` into its own module (`prompts.py`) that `api.py` and both eval scripts now share, so the harness runs the real configuration instead of a stand-in for it.

Second, and more consequential: `run_ragas.py`'s dependencies cannot be installed into this project's actual virtualenv. `ragas` (the version whose API this script's `faithfulness` / `answer_relevancy` / `context_precision` imports match) requires `langchain-core<0.3`; `langgraph`, `langchain-pinecone`, and `langchain-deepseek` all require `langchain-core>=1.x`. Installing it doesn't just fail cleanly — it silently downgrades `langchain-core` and breaks the real application, which is exactly what happened here: `pytest` started throwing `TypeError: Reviver.__init__() got an unexpected keyword argument 'allowed_objects'` from deep inside `langgraph`'s checkpoint serializer, immediately after the install. Recovered by reinstalling from `requirements.txt` and removing `ragas`/`datasets` again. A newer `ragas` (0.4.x) avoids the version conflict but fails to import for an unrelated reason (a `langchain_community.chat_models.vertexai` reference that no longer exists), and uses a different metrics API this script doesn't match anyway. Running Ragas here for real would need its own fully isolated virtualenv with an old, pinned stack — not an addition to this project's `requirements.txt`. Documented at the top of `run_ragas.py` and left unrun rather than forced.

`run_evals.py` only needs `pandas`, which has no such conflict — added as `requirements-eval.txt` (kept separate from `requirements.txt` so the production Docker image doesn't carry eval-only weight) and actually run, with the `SYSTEM_PROMPT` fix in place, against the real DeepSeek + Pinecone stack, post-re-ingest, post every fix in this document:

First pass: 5/5 questions PASS (100%), spot-checked by hand. Good signal, but 5 questions all drawn from roughly the same shape (single-fact lookups, mostly 2026-only) doesn't stress the parts of the system that actually broke earlier in this investigation — multi-year comparisons, and anything outside the regulation corpus.

### Expanding the golden set, and getting more than one number out of it

Grew `f1_golden.csv` from 5 to 15 questions across four categories — `rule` (simple and hard single-year lookups), `comparison` (across 2025/2026, the exact query shape that caused the recursion-loop failures earlier), and `web_search` (general F1 facts outside the regulation corpus, exercising `search_web` instead of `search_knowledge_base`).

True retrieval precision and recall would need someone to hand-label, for each question, exactly which passages in the corpus *should* be retrieved — real annotation work this golden set doesn't have. What's actually computable from what the system already produces: `run_evals.py` now also captures, per question, the reranker scores `retrieval.py` was already logging (a **retrieval-precision proxy** — the fraction of retrieved chunks the cross-encoder itself judges relevant), a **faithfulness** check reimplemented directly as a second judge call (Ragas's idea, without needing the unusable `ragas` package), an **answer-relevancy** check, a **citation-compliance** check (regex, no LLM call needed), and efficiency numbers (rounds, real search-budget usage, latency) — all broken out by category.

First real run surfaced a bug in the harness itself, immediately: every comparison question scored 0% faithfulness, while every rule question scored close to 100%. Reading the actual flagged answers by hand, they looked well-grounded — one even honestly said "the 2025 DRS sporting regulation specifics weren't fully captured" rather than inventing something. The cause was the faithfulness judge prompt: a comparison answer necessarily *computes* something across two searches (e.g. "nearly threefold increase" from 120kW to 350kW), and the strict "flag any claim not in the context" instruction was treating a valid computed delta as an unsupported claim, since the phrase "threefold increase" itself never appears verbatim in either year's retrieved text. Fixed by explicitly telling the judge that a computed comparison is faithful as long as the individual numbers it's built from are each grounded — re-ran, comparison faithfulness went from 0% to 100%.

**Final numbers** (real DeepSeek + Pinecone stack, current re-ingested index, every fix in this document applied):

| Metric | Overall | rule | comparison | web_search |
|---|---|---|---|---|
| Correctness (LLM-judge) | 86.7% | 88.9% | 75.0% | 100% |
| Faithfulness | 86.7% | 88.9% | 100% | 50%* |
| Answer relevancy | 100% | 100% | 100% | 100% |
| Citation compliance | 93.3% | — | — | — |
| Retrieval precision proxy | ~32% | — | — | — |
| Avg. rounds / question | 1.9 | 1.8 | 1.75 | 2.5 |
| Avg. real KB searches / question | 2.4 | — | — | — |
| Search-budget cap hit rate | **0%** | — | — | — |
| Avg. latency | 12.2s | 10.7s | 15.0s | 13.6s |

\* The one web_search "unfaithful" flag is the points-system question, where the model explicitly said the exact points table "was not directly retrieved" from the regulation corpus and answered from general knowledge instead of fabricating a citation — technically ungrounded in what it searched, but the honest behavior, not a hallucination.

What's actually worth taking from this:
- **The search-budget cap never had to intervene, across 30 question-runs (15 questions × 2 full eval passes).** The recursion-loop failures chased earlier in this document really were specific to one query shape/phrasing, not a general problem with legitimate comparison questions.
- **Retrieval precision proxy sitting around 32% is a real, quantified version of the "reranker score computed but not used" gap** already called out below — roughly two of every three chunks in a typical top-8 aren't judged relevant by the reranker's own scoring, even though the final answers are still mostly correct. There's real headroom here.
- **Individual question grades aren't perfectly stable run-to-run** — the same weight-comparison question passed in one run and failed in the next, purely from LLM non-determinism. The aggregate trends (~87% accuracy, ~87% faithfulness, ~32% precision proxy, holding steady across two independent full runs) are the trustworthy signal; any single question's PASS/FAIL from one run is a sample, not gospel.
- **Both correctness "failures" trace to the same root cause, and it's probably the ground truth, not the system**: the golden set says the 2026 minimum weight is a flat 770kg (the commonly-cited round figure); the system consistently cites the regulation's actual, more precise, session-dependent figures (724kg / 726kg, with tyre mass handled as a separate additive term) straight from the source PDF. That's the system citing the primary source more precisely than the golden answer it's being graded against — a labeling problem in the eval, not a retrieval or generation bug. Writing accurate ground truth for a precise regulatory domain is genuinely hard, and it's worth remembering that a "FAIL" here means "check the ground truth" first, not just "the system is wrong."
...
- ~~The exact "Compare 2025 DRS with 2026 Active Aero" query reliably failed to produce an answer, hitting `RECURSION_LIMIT=10`.~~ **Fixed, in three passes, prompt-based then code-enforced.** First pass (a per-year search cap) reduced but didn't eliminate it — failed again on the very next live use, one round longer than local testing had seen, since LLM output isn't perfectly deterministic even at `temperature=0` and the cap had zero margin. Second pass: raised `RECURSION_LIMIT` to 16 and replaced the per-year rule with a flat "4 searches total" prompt budget, after retrieval-score logging showed the model decomposes comparisons by *facet* rather than by year. Third pass: the prompt-based "4" was still just a request the model could (and did — 6-8 calls observed) reason past, so `MAX_KB_CALLS_PER_TURN` is now enforced in code — `agent_node` stops offering `search_knowledge_base` once the count is hit, and `tool_node` backstops any single round that bundles more calls than the remaining budget. Verified directly against retrieval-layer logs (not just the frontend telemetry): exactly 4 real searches per run, every run, 0 failures.
- ~~The reranker score is logged but not *used* — weak results are indistinguishable in format from a strong match.~~ **Fixed.** `status_for` (`retrieval.py`) now tags any chunk the cross-encoder scores below `LOW_CONFIDENCE_THRESHOLD` (0.0) with `[[LOW CONFIDENCE MATCH — relevance score X.XX]]` in its context entry, and the system prompt (rule 7) tells the agent what that means: treat it as background, don't cite it as authoritative, and say plainly the regulations don't appear to specify something rather than presenting a weak match as a firm answer. Verified live against a question the corpus genuinely doesn't answer (PU manufacturer warranty terms) — the agent used its full search budget, correctly identified that what it found (an IP-infringement warranty clause) didn't actually answer the question asked, and said so explicitly instead of misciting the wrong article or fabricating a number. A clean, isolated sweep (confidence tagging on, budget held at 4, only `RETRIEVAL_K` varied — 3, 5, 6, 8) confirmed 8 is the right value: every metric moved monotonically worse as K dropped — faithfulness 93.3% → 86.7% → 80.0% → 60.0%, citation compliance 93.3% → 93.3% → 86.7% → 60.0%, budget-cap pressure jumping as soon as K left 8. Confidence tagging targets the actual problem (weak results look identical to strong ones) without touching how much context legitimate multi-facet questions get.
- **`run_ragas.py` cannot currently be run** — its dependencies conflict at the version level with this project's actual `langgraph`/`langchain-core` stack (see "Checking the actual quality" above for the full story, including the time installing it downgraded `langchain-core` and broke the real app). Would need a separate, isolated virtualenv to ever run.
- **The golden eval set is now 15 questions across rule/comparison/web_search categories, still short of the 20 a resume-facing description of this project claims** — closer than the original 5, not there yet.
- **True retrieval precision/recall are still unmeasured** — the retrieval-precision proxy `run_evals.py` reports is real and computed from the reranker's own scoring, but it isn't the same thing as precision/recall against human-labeled relevant passages, which this golden set doesn't have. Building that would mean hand-annotating, per question, exactly which corpus passages are relevant — real work, not a script.
