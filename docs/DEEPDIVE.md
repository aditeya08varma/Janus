...
- **The reranker score is now logged (`retrieval.py`) but still not *used*.** Two of six searches in one investigated turn came back with every top-8 chunk scoring negative (the cross-encoder's own signal that nothing retrieved was relevant), and those chunks were still returned to the agent, indistinguishable in format from a strong match. The natural next step — tagging weak results with something like `[[LOW CONFIDENCE]]` so the agent can answer with an honest caveat instead of searching again — is scoped out but not built.
...
- **True retrieval precision/recall are still unmeasured** — the ~32% retrieval-precision proxy `run_evals.py` reports is real and computed from the reranker's own scoring, but it isn't the same thing as precision/recall against human-labeled relevant passages, which this golden set doesn't have. Building that would mean hand-annotating, per question, exactly which corpus passages are relevant — real work, not a script.
