...
### One more thing this surfaced, still open

Running the exact same live query repeatedly also surfaced a separate, pre-existing issue unrelated to any of the above: on this specific comparison question, DeepSeek reliably decides it needs "more specific details" and re-searches five times in a row — each round genuinely fast now — without ever synthesizing a final answer, until it trips the existing `RECURSION_LIMIT=10` safety cap and the turn ends in a generic error instead of a response. This isn't a crash and isn't a regression from anything in this pass — the recursion limit is old code doing exactly its job, gracefully aborting a runaway loop — but it means the exact query wired into the sidebar's "Aero Transition" preset button reliably fails to produce an answer at all. That's a model-behavior/prompt problem, not a latency one, and it's outside what this pass set out to fix. Flagged here rather than fixed, since it needs actual iteration on the system prompt or the tool's return format, not a code change I'd want to make in this same sitting.
...
- **The exact "Compare 2025 DRS with 2026 Active Aero" query — the one wired into the sidebar's own preset button — reliably fails to produce an answer.** DeepSeek decides it needs "more specific details" and re-searches five times in a row on this question, never converging, until it trips `RECURSION_LIMIT=10` and the turn ends in a generic error. Confirmed reproducible on a fresh session, not a one-off. This is a model-behavior/prompt problem, not a latency one — the searches themselves are fast now — and needs iteration on the system prompt or the tool's return format rather than a code fix.
