...
- ~~The exact "Compare 2025 DRS with 2026 Active Aero" query reliably failed to produce an answer, hitting `RECURSION_LIMIT=10`.~~ **Fixed, in two passes.** First pass (a per-year search cap) reduced but didn't eliminate it — it failed again on the very next live use, one round longer than local testing had seen, because LLM output isn't perfectly deterministic even at `temperature=0` and the cap had zero margin. Second pass: raised `RECURSION_LIMIT` to 16 for headroom, and — after logging the actual reranker scores driving each retrieval and discovering the model decomposes comparisons by *facet* (activation criteria, speed threshold, gap rule as separate searches) rather than by year — replaced the per-year rule with a flat "at most 4 searches total this turn" budget. Six live runs against the real stack across both passes: 0 failures. Still not fully disciplined — the model uses 6-8 calls against a stated budget of 4 — but the failure mode itself hasn't recurred.
