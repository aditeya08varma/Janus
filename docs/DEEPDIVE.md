...
- ~~The reranker score is logged but not *used* — weak results are indistinguishable in format from a strong match.~~ **Fixed.** `status_for` (`retrieval.py`) now tags any chunk the cross-encoder scores below `LOW_CONFIDENCE_THRESHOLD` (0.0) with `[[LOW CONFIDENCE MATCH — relevance score X.XX]]` in its context entry, and the system prompt (rule 7) tells the agent what that means: treat it as background, don't cite it as authoritative, and say plainly the regulations don't appear to specify something rather than presenting a weak match as a firm answer. Verified live against a question the corpus genuinely doesn't answer (PU manufacturer warranty terms) — the agent used its full search budget, correctly identified that what it found (an IP-infringement warranty clause) didn't actually answer the question asked, and said so explicitly instead of misciting the wrong article or fabricating a number. Considered reducing `RETRIEVAL_K` from 8 to 3 instead, as a blunter way to raise the relevance density of what's returned — A/B tested it directly against the golden eval set first: precision proxy did roughly double (32% → 68%), but the model compensated for thinner individual searches by searching more (avg rounds 1.9 → 2.5, real searches 2.4 → 3.1), pushed 13.3% of questions into the search-budget cap (up from 0%), and faithfulness got *worse* (86.7% → 80.0%), not better — reverted. Confidence tagging targets the actual problem (weak results look identical to strong ones) without touching how much context legitimate multi-facet questions get.
